 ...
 */
@RestController
@RequestMapping("/groupApprovalLevelsConfig")
public class ApprovalLevelConfigController {

    private final Logger LOG = Logger.getLogger(ApprovalLevelConfigController.class);
    ...
    @ResponseBody
    public ResponseEntity<?> create(@RequestHeader(value = "Authorization") String authKey, @RequestBody GroupApprovalLevelsConfig groupApprovalLevelsConfig) {
        ResponseEntity<?> res = null;
        try {
            Token token = globalFunctions.parseJWT(authKey);

            groupApprovalLevelsConfig.setCreatedBy(Integer.parseInt(token.getUserid()));

            Object test = crudService.save(groupApprovalLevelsConfig);
    ...
    public ResponseEntity<?> findById(@RequestHeader(value = "Authorization") String authKey, @RequestParam("id") String id) {
        ...
        try {
            ...
            GroupApprovalLevelsConfig entity = crudService.findEntity(Integer.parseInt(id), GroupApprovalLevelsConfig.class);
    ...
    @ResponseBody
    public ResponseEntity<?> update(@RequestHeader(value = "Authorization") String authKey, @RequestBody GroupApprovalLevelsConfig groupApprovalLevelsConfig) {
        ResponseEntity<?> res = null;
        try {
            Token token = globalFunctions.parseJWT(authKey);
            groupApprovalLevelsConfig.setUpdatedBy(Integer.parseInt(token.getUserid()));

            crudService.saveOrUpdate(groupApprovalLevelsConfig);

            ApiResponse SUCCESS = responseCodes.SUCCESS;
            SUCCESS.setEntity(groupApprovalLevelsConfig);
            res = new ResponseEntity<>(SUCCESS, HttpStatus.OK);
        } catch (Exception ex) {
            ex.printStackTrace();
            LOG.error("Error Occured:" + ex.getMessage());
    ...
    @ResponseBody
    public ResponseEntity<?> delete(@RequestHeader(value = "Authorization") String authKey, @RequestBody GroupApprovalLevelsConfig groupApprovalLevelsConfig) {
        ResponseEntity<?> res = null;
    ...
    public ResponseEntity<?> findByGroupId(@RequestHeader(value = "Authorization") String authKey, @RequestParam("groupId") String groupId, @RequestParam("start") int start, @RequestParam("end") int end) {
        ...
        try {
            ...
            String q = "select r from GroupApprovalLevelsConfig r where groupId=:groupId";
            Map<String, Object> params = new HashMap<String, Object>();
            params.put("groupId", groupId);
            List<GroupApprovalLevelsConfig> entity = crudService.fetchWithHibernateQuery(q, params, start, end);
