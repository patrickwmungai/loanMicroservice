 ...
 */
@RestController
@RequestMapping("/groupApprovalLevel")
public class ApprovalLevelsController {

    private final Logger LOG = Logger.getLogger(ApprovalLevelsController.class);
    ...
    @ResponseBody
    public ResponseEntity<?> create(@RequestHeader(value = "Authorization") String authKey, @RequestBody GroupApprovalLevel groupApprovalLevel) {
        ResponseEntity<?> res = null;
        try {
            Token token = globalFunctions.parseJWT(authKey);

            groupApprovalLevel.setCreatedBy(Integer.parseInt(token.getUserid()));

            Object test = crudService.save(groupApprovalLevel);
    ...
    public ResponseEntity<?> findById(@RequestHeader(value = "Authorization") String authKey, @RequestParam("id") String id) {
        ...
        try {
            ...
            GroupApprovalLevel entity = crudService.findEntity(Integer.parseInt(id), GroupApprovalLevel.class);
    ...
    @ResponseBody
    public ResponseEntity<?> update(@RequestHeader(value = "Authorization") String authKey, @RequestBody GroupApprovalLevel groupApprovalLevel) {
        ResponseEntity<?> res = null;
        try {
            Token token = globalFunctions.parseJWT(authKey);
            groupApprovalLevel.setUpdatedBy(Integer.parseInt(token.getUserid()));

            crudService.saveOrUpdate(groupApprovalLevel);

            ApiResponse SUCCESS = responseCodes.SUCCESS;
            SUCCESS.setEntity(groupApprovalLevel);
            res = new ResponseEntity<>(SUCCESS, HttpStatus.OK);
        } catch (Exception ex) {
            ex.printStackTrace();
            LOG.error("Error Occured:" + ex.getMessage());
    ...
    @ResponseBody
    public ResponseEntity<?> delete(@RequestHeader(value = "Authorization") String authKey, @RequestBody GroupApprovalLevel groupApprovalLevel) {
        ResponseEntity<?> res = null;
...
//            String q = "select r from GroupApprovalLevel r ";
//            List<GroupApprovalLevel> entity = crudService.fetchWithHibernateQuery(q, Collections.EMPTY_MAP, start, end);
            String addedQueryfilter = "";
            Map<String, Object> params = new HashMap<>();
            List<GroupApprovalLevel> entity = null;
            if (token.getIsadminagent().equals("NO")) {
                addedQueryfilter = " WHERE r.groupId =:id ";
                params = Collections.singletonMap("id", token.getGroupID());
            }

            String q = "select r from GroupApprovalLevel r  " + addedQueryfilter;
            if (token.getIsadminagent().equals("NO")) {
    ...
    public ResponseEntity<?> findByGroupId(@RequestHeader(value = "Authorization") String authKey, @RequestParam("groupId") String groupId, @RequestParam("start") int start, @RequestParam("end") int end) {
        ...
        try {
            ...
            String q = "select r from GroupApprovalLevel r where groupId=:groupId";
            Map<String, Object> params = new HashMap<String, Object>();
            params.put("groupId", groupId);
            List<GroupApprovalLevel> entity = crudService.fetchWithHibernateQuery(q, params, start, end);
