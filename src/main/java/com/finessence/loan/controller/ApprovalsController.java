...
public class ApprovalsController {
    ...
    private final Logger LOG = Logger.getLogger(ApprovalsController.class);
    ...
    @ResponseBody
    public ResponseEntity<?> create(@RequestHeader(value = "Authorization") String authKey, @RequestBody ApprovalsDone approvalsDone) {
        ResponseEntity<?> res = null;
        try {
            Token token = globalFunctions.parseJWT(authKey);

            approvalsDone.setApproverId(Integer.parseInt(token.getUserid()));

            Object test = crudService.save(approvalsDone);
    ...
    public ResponseEntity<?> findById(@RequestHeader(value = "Authorization") String authKey, @RequestParam("id") String id) {
        ...
        try {
            ...
            ApprovalsDone entity = crudService.findEntity(Integer.parseInt(id), ApprovalsDone.class);
    ...
    @ResponseBody
    public ResponseEntity<?> update(@RequestHeader(value = "Authorization") String authKey, @RequestBody ApprovalsDone approvalsDone) {
        ResponseEntity<?> res = null;
        try {
            Token token = globalFunctions.parseJWT(authKey);

            crudService.saveOrUpdate(approvalsDone);

            ApiResponse SUCCESS = responseCodes.SUCCESS;
            SUCCESS.setEntity(approvalsDone);
            res = new ResponseEntity<>(SUCCESS, HttpStatus.OK);
        } catch (Exception ex) {
            ex.printStackTrace();
            LOG.error("Error Occured:" + ex.getMessage());
    ...
    @ResponseBody
    public ResponseEntity<?> delete(@RequestHeader(value = "Authorization") String authKey, @RequestBody ApprovalsDone approvalsDone) {
        ResponseEntity<?> res = null;
...
//            String q = "select r from ApprovalsDone r ";
//            List<ApprovalsDone> entity = crudService.fetchWithHibernateQuery(q, Collections.EMPTY_MAP, start, end);

            String addedQueryfilter = "";
            Map<String, Object> params = new HashMap<>();
            List<ApprovalsDone> entity = null;
            if (token.getIsadminagent().equals("NO")) {
                addedQueryfilter = " WHERE r.groupId =:id ";
                params = Collections.singletonMap("id", token.getGroupID());
            }

            String q = "select r from ApprovalsDone r  " + addedQueryfilter;
            if (token.getIsadminagent().equals("NO")) {
    ...
    public ResponseEntity<?> findByGroupId(@RequestHeader(value = "Authorization") String authKey, @RequestParam("groupId") String groupId, @RequestParam("start") int start, @RequestParam("end") int end) {
        ...
        try {
            ...
            String q = "select r from ApprovalsDone r where groupId=:groupId";
            Map<String, Object> params = new HashMap<String, Object>();
            params.put("groupId", groupId);
            List<ApprovalsDone> entity = crudService.fetchWithHibernateQuery(q, params, start, end);
    ...
    @RequestMapping(value = "/findByTypeAndRecordId", method = RequestMethod.GET)
    @ResponseBody
    public ResponseEntity<?> findByTypeAndRecordId(@RequestHeader(value = "Authorization") String authKey, @RequestParam("type") String type, @RequestParam("recordId") Integer recordId, @RequestParam("start") int start, @RequestParam("end") int end) {
        ResponseEntity<?> res = null;
        //return record set and total count of rows on the entity
        try {
            Token token = globalFunctions.parseJWT(authKey);

            String q = "select r from ApprovalsDone r where type=:type and recordId=:recordId";
            Map<String, Object> params = new HashMap<String, Object>();
